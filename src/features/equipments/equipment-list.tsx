...
export function EquipmentList({
            ...
            <div className="mt-6 space-y-4">
                <div className="grid gap-4 md:grid-cols-2">
                    {[1, 2, 3, 4].map((i) => (
                        <div
                            key={i}
                            className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900/40 shadow-xs"
                        >
                            <div className="space-y-2">
                                <div className="h-4 w-40 animate-pulse rounded bg-slate-200 dark:bg-slate-800" />
                                <div className="h-3 w-28 animate-pulse rounded bg-slate-200/60 dark:bg-slate-800/60" />
                            </div>
                        </div>
                    ))}
                </div>
    ...
    if (equipments.length === 0) {
        ...
        return (
            <div className="mt-6 flex flex-col items-center justify-center rounded-xl border border-dashed border-slate-300 bg-white/50 px-6 py-12 text-center dark:border-slate-800 dark:bg-slate-900/30">
        ...
        <section className="mt-6 space-y-4">
            ...
            <div className="grid gap-3 md:grid-cols-2">
                {equipments.map((equipment) => (
                    <article
                        key={equipment.id}
                        className="flex flex-col justify-between rounded-xl border border-slate-200 bg-white p-4 shadow-xs transition-colors hover:border-slate-300 dark:border-slate-800 dark:bg-slate-900/60 dark:hover:border-slate-700"
                    >
                        <div>
                            ...
                            <div className="flex items-center gap-3">
                                {equipment.photo_url && (
                                    <EquipmentPhoto
                                        ...
                                        alt={equipment.name}
                                        className="h-10 w-10 shrink-0 rounded-lg border border-slate-200 object-cover dark:border-slate-800"
                                    />
                                )}

                                <div className="min-w-0 flex-1 space-y-0.5 text-xs text-slate-600 dark:text-slate-400">
                                    <p>
                                        <span className="text-slate-400 dark:text-slate-500">Patrimônio: </span>
                                        <span className="font-mono text-slate-800 font-semibold dark:text-slate-300">{equipment.patrimony_code}</span>
                                    </p>
                                    <p>
                                        <span className="text-slate-400 dark:text-slate-500">Local: </span>
                                        <span className="text-slate-800 dark:text-slate-300">{equipment.location}</span>
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="mt-3 pt-2.5 border-t border-slate-100 flex items-center justify-between gap-2 dark:border-slate-800/80">
                            <Link
                            ...
                            <div className="flex items-center gap-2">
                                <button
                                    ...
                                    className="rounded px-2 py-1 text-xs text-slate-600 hover:bg-slate-100 transition-colors dark:text-slate-300 dark:hover:bg-slate-800 cursor-pointer"
                                >
                                    Editar
                                </button>
                                ...
                                <button
                                    ...
                                    className="rounded px-2 py-1 text-xs text-red-600 hover:bg-red-50 transition-colors disabled:opacity-50 dark:text-red-400 dark:hover:bg-red-950/30 cursor-pointer"
                                >
